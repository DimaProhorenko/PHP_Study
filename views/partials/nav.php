    ...
    <div class="flex h-16 items-center justify-between">
      ...
      <div class="hidden md:block">
        <div class="ml-4 flex items-center md:ml-6">
          < <!-- Profile dropdown -->
            <div class="flex items-center gap-6 ml-3">
              <div>
                <?php if ($_SESSION['user'] ?? false) : ?>
                  <img class="h-8 w-8 rounded-full" src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80" alt="">
                <?php else : ?>
                  <a href="/login" class="text-white text-md border border-white px-6 py-2 rounded-full hover:bg-indigo-500 hover:border-indigo-500 transition-colors">Login</a>
                <?php endif ?>
              </div>
              <?php if ($_SESSION['user'] ?? false) : ?>
                <div>
                  <form action="/logout" method="post">
                    ...
                    <button class="text-white hover:underline">Log out</button>
                  </form>
                </div>
              <?php endif ?>
            </div>
